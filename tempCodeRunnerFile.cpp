...
void atmClass::add(INFO user){
    ...
    temp->user.contact_num = user.contact_num;
    temp->next = p;
    while(p!=NULL){
        q = p;
        p = p->next;
...
void atmClass::save_record(){
    p=n;
    fstream doc;
    doc.open("alldata.txt", ios::out);
    while (p!=NULL){
        doc<<p->user.acc_number<<'\n';
        doc<<p->user.birthday<<'\n';
        doc<<p->user.contact_num<<'\n';
        p=p->next;
    }
    doc.close();
}
...
void atmClass::retrieve_record(){
    ...
    if(!doc){
        ...
        while(true){
            ...
            doc>>user.acc_number>>user.birthday>>user.contact_num;
            doc.ignore();
